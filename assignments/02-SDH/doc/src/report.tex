...
\runningtitle{Assignment \#02 — SDH} % Short title
...
\section{Arduino UNO Circuit}
\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{assets/imgs/ass02-SDH Circuit.png}
    \caption{Screenshot of the circuit made with TinkerCad}
...
Considering all of that, we defined the following tasks:
\begin{enumerate}
    \item \texttt{BlinkingLedTask}
    \item \texttt{HangarDoorTask}
    \item \texttt{TemperatureMonitoringTask}
    \item \texttt{HangarMainTask}
\end{enumerate}

In order to orchestrate the progress of tasks as time goes on, a \textbf{scheduler} is mandatory. Following the Round Robin scheduling technique, a period of time (\textit{called "quantum"}) is assigned at each task. During this period of time the task can use the CPU. If the task finishes its job before the ending of its quantum, it should leave the system. Otherwise, it goes back into the queue of tasks to be executed.
It's up to the scheduler to choose the order of tasks to execute, regardless of their priority.

The tricky challenge is to choose a quantum of time \textbf{not too small} to make the system unresponsive and loose events and \textbf{not too big} to incur in overrun exceptions. 
\subsubsection{BlinkingLedTask}
\paragraph{Hardware needed}
\begin{itemize}
    \item 1x LED
\end{itemize}
\paragraph{Quantum}
0.5 seconds
\paragraph{Goal}
The meaning of this task — as the name suggests — is to handle the blinking of LED \texttt{L2} during take-offs and landings of the drone.
It needs to read the state of the drone in order to decide whether to start blinking or not.

\subsubsection{HangarDoorTask}
\paragraph{Hardware needed}
\begin{itemize}
    \item 1x Servo motor
\end{itemize}
...
0.5 seconds
\paragraph{Goal}
The task that opens and closes the hangar door (represented by the Servo motor). It reads the drone state in order to start opening the door, and the hangar state to understand wheter the system is in alarm and thus close the door. It is responsible for changing the \textbf{isDoorOpen}, one of the shared variables used to let tasks communicate.

\subsubsection{TemperatureMonitoringTask}
\paragraph{Hardware needed}
\begin{itemize}
    \item 1x Temperature sensor
    \item 1x Button (Reset button)
    \item 1x LED
    \item 1x LCD
\end{itemize}
\paragraph{Quantum}
0.5 seconds
\paragraph{Goal}
It reads the drone state and understands in what moments it should check if the heat inside the hangar exceeds some thresholds. It is responsible for changing the \textbf{hangar state}, one of the three shared variables used to let tasks communicate.

\subsubsection{HangarMainTask}
\paragraph{Hardware needed}
\begin{itemize}
    \item 1x LED
    \item 1x Sonar (Drone Distance Detector)
    \item 1x Pir (Drone Presence Detector)
    \item 1x LCD 
\end{itemize}
\paragraph{Quantum}
0.5 seconds
\paragraph{Goal}
The most complex task of all. It relates directly to the drone, understanding its position during time (inside or outside). It is responsible for changing the \textbf{drone state}, one of the three shared variables used to let tasks communicate.

\subsubsection{UML State Diagram of the tasks FSMs}
% Source - https://stackoverflow.com/a/2739710
% Posted by Mica, modified by community. See post 'Timeline' for change history
% Retrieved 2026-02-08, License - CC BY-SA 3.0
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\linewidth]{assets/imgs/ass02-SDH Circuit.png}
    \caption{Screenshot of the GUI}
    \label{fig:placeholder1}
\end{figure}

\subsection{How to run the program}
In order to start and work properly, the application needs \textbf{Arduino to be connected to serial port}. Default port is set to \texttt{/dev/cu.usbmodem1201}.
...
\subsubsection{Setting the Serial Port}
